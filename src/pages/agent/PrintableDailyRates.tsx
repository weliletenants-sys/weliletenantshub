...
const PrintableDailyRates = () => {
  const [ratesData, setRatesData] = useState(() => {
    const saved = localStorage.getItem("printable-rates-agent");
  ...
  useEffect(() => {
    localStorage.setItem("printable-rates-agent", JSON.stringify(ratesData));
  }, [ratesData]);
  ...
  return (
    <AgentLayout currentPage="/agent/printable-rates">
      <div className="container max-w-4xl mx-auto p-4 space-y-6">
        <div className="print:hidden space-y-4">
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-3xl font-bold">Printable Daily Rates</h1>
              <p className="text-muted-foreground">Quick reference for customer consultations</p>
            </div>
          </div>
